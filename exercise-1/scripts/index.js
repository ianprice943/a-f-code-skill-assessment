...
function generateHandlebarTemplate() {
    const template = "<li><article><h3>{{this.name}}</h3><div><img src='{{this.avatar}}' alt='{{this.name}}s avatar'></div><button id='toggle{{this.id}}'>Toggle ID and Date</button><div id='wrapper{{this.id}}' class='hide'><p>{{this.createdAt}}</p><p>{{this.id}}</p></div></article></li>"
    return template;
}
